/************************************************************************
    ...
    Copyright (C) 1996, 2014  John Elliott <seasip.webmaster@gmail.com>
...
const char *tio_mktemp(TIO_FILE **pfp, TIO_FORMAT fmt)
{
	...
	*pfp = NULL;
	...
	if (!self->fp) self->fp = fopen(self->filename, "w+b");
#endif
	if (!self->fp)
	{
		sprintf(msgbuf, "Failed to create temporary tape file %s.",
			self->filename);
		free(self);
		return msgbuf;
	}
